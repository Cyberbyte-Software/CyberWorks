        ...
        foreach ($plugins as &$plugin) {
            ...
            if ($key !== false) {
                if (in_array($plugin['dirname'], $settings['plugins'])) {
                    if ($plugin['version'] > $installed[$key]['version']) echo '<small style="padding-left: 6px;"><span class="label label-info">' . $lang['updateAvalible'] . '</span></small>'; // Update avalible
                    else echo '<small style="padding-left: 6px;"><span class="label label-success">' . $lang['active'] . '</span>'; // Active plugin
                } elseif (in_array($plugin['dirname'], $files)) echo '<small style="padding-left: 6px;"><span class="label label-default">' . $lang['installed'] . '</span></small>'; // Installed not active
            }
            echo '</h3><p><strong>' . $plugin['author'] . '</strong> - ' . $plugin['description'] . '</p>';

            if ($key !== false) {
                if (in_array($plugin['dirname'], $settings['plugins'])) {
                    if ($plugin['version'] > $installed[$key]['version']) echo '<div><a href="?install=' . $plugin['dirname'] . '" class="btn btn-primary" role="button">' . $lang['update'] . '</a>'; // Update avalible
                    else echo '<p><a href="?deactivate=' . $plugin['dirname'] . '" class="btn btn-primary" role="button">' . $lang['deactivate'] . '</a>'; // Active plugin
                } elseif (in_array($plugin['dirname'], $files)) echo '<p><a href="?activate=' . $plugin['dirname'] . '" class="btn btn-primary" role="button">' . $lang['activate'] . '</a>'; // Installed not active
            } else echo '<p><a href="?install=' . $plugin['dirname'] . '" class="btn btn-primary" role="button">' . $lang['install'] . '</a>'; // Not installed

            if (isset($plugin['authurl'])) echo '<a style="margin-left: 5px;" href="' . $plugin['authurl'] . '" class="btn btn-default" role="button">' . $lang['visitSite'] . '</a>';
            if (in_array($plugin['dirname'], $files)) echo '<a style="float: right;" href="?delete=' . $plugin['dirname'] . '" class="btn btn-danger" role="button">' . $lang['delete'] . '</a></div>';
            echo '</div></div></div>';
        }

        echo '</div></div></form>';
    } else echo '<h2>' . $lang['notverified'] . '</h2>';
} else echo '<h2>' . $lang['noPlugin'] . '</h2>';