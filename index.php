...
if (file_exists('config/settings.php')) {
    ...
    if (!$db_connection->connect_errno) {
        if ($login->isUserLoggedIn() == true) {
            if (!isset($_SESSION['formtoken'])) formtoken::generateToken();
            if ($_SESSION['formtoken'][1] < time() - 600) formtoken::generateToken();
            ...
            if ($currentPage == '' || $currentPage == 'index' || $currentPage == 'dashboard') {
                ...
                if ($_SESSION['server_type'] == 'life' && !$_SESSION['steamsignon']) {
                    ...
                    if ($currentPage == 'messages') {
                        ...
                        if ($_SESSION['permissions']['edit']['gangf']) {
                            if ($query) {
            ...
            if ($currentPage == 'newdb') {
                if ($_SESSION['permissions']['super_admin']) {
                    if (!isset($_SESSSION['sudo']) || isset($_POST['sudo'])) {
                        pass();
                    } else {
                        if ($_SESSSION['sudo'] > now() + 10800) pass(1); else {
            ...
            if ($currentPage == '2factor' && isset($_SESSION['user_email'])) $page = 'views/core/2factor.php';
...
} else include ('views/firstTime.php');